...
4. **Marker-based annotation** via `annotate_celltype_ulm(adata, output_dir, marker_dict)`:
   - Load marker genes from `${CIIM_DATALAKE_DIR}/prior/marker_genes.json` (10 major lineages, 15 minor subtypes)
...
7. **TF filter** — restrict `source` to 1,638 human TFs in `${CIIM_DATALAKE_DIR}/prior/tf_all.csv`
...
### Prior Files (`${CIIM_DATALAKE_DIR}/prior/`)
