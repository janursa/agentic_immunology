#!/usr/bin/env python3
"""Structured feedback store: memory_bank/memory_blob.jsonl.
...
add-tag  -> extend the fixed issue-tag taxonomy in memory_bank/issue_tags.json.
"""
import argparse
import datetime
import json
...
# Agent names are registered in two places: the host's own agents/list.md and
# egad's agents/list.md (egad ships as a submodule one level down).
LIST_FILES = (MEMORY_DIR.parent / "agents" / "list.md", MEMORY_DIR.parent / "egad" / "agents" / "list.md")
...
def _demo() -> None:
    import tempfile
    global TAGS_FILE, BLOB_FILE, LIST_FILES
    real_tags, real_blob, real_lists = TAGS_FILE, BLOB_FILE, LIST_FILES
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        TAGS_FILE = tmp / "issue_tags.json"
        BLOB_FILE = tmp / "memory_blob.jsonl"
        listmd = tmp / "list.md"
        LIST_FILES = (listmd,)
        TAGS_FILE.write_text(json.dumps({"stat_approach": "statistical method issues"}))
        ...
        got = retrieve(agent="data_analyst_agent")
        assert len(got) == 1 and got[0]["lesson"] == entry["lesson"]
        assert retrieve(agent="orchestrator") == []  # EXTRA_AGENTS is valid for add, not retrieved here
        try:
            add_entry("stat_approach", ["nonexistent_agent"], "t", "Situation: x. Lesson: y.")
            raise AssertionError("expected ValueError for unknown agent")
        except ValueError:
            pass
        add_tag("new_tag", "a new kind of issue")
        assert json.loads(TAGS_FILE.read_text())["new_tag"] == "a new kind of issue"
    TAGS_FILE, BLOB_FILE, LIST_FILES = real_tags, real_blob, real_lists
    print("ok")
