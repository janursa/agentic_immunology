...
| `egad_supp` | sonnet | Read, Write, Edit, Bash, Grep, Glob, Agent, SendMessage | On-demand. Supplementary work linked to an egad run — paper curation (delegates to `curate_paper`), rubric scoring (delegates to `rubric_agent`), and post hoc analysis on an already-finished run. Always writes under `temp/`; a post hoc analysis of an existing run reuses that run's own task-name. Curates user feedback on its own output into `memory_bank/memory_blob.jsonl`. |
