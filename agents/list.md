...
| `feedback_analyser` | sonnet | Read, Grep, Glob | On-demand only (user request, not part of the numbered loop). Curates `knowhow/memory_blob.jsonl`: dedups each entry against `knowhow/guardrail.md`, drafts a promotion bullet (NEW/REVISE/DUPLICATE). Never writes either file — orchestrator relays proposals to the user and performs the promotion itself on approval. |
