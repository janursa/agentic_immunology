...
| `peer_reviewer_agent` | opus | Read, Write, Grep, Glob, WebSearch, WebFetch | Critical referee with three modes. METHOD-REVIEW (user-triggered, post-analysis): audits actual code for correctness, leakage, batch handling, stats, reproducibility; returns PASS/REVISE. DESIGN-REVIEW (complex tasks, pre-execution): sanity-checks the draft design. RESULTS-REVIEW (default, per cycle): evaluates results against the criteria, writes `peer_review.md`, returns ACCEPT/REVISE/CANNOT-MEET. |
