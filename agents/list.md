...
|---|---|---|---|
| `study_designer_agent` | sonnet | Read, Write, Bash, Grep, Glob, WebSearch, WebFetch | Designs the study plan — the numbered plan, checkpoints, and evaluation procedure — at the start of every task, plus design-review revisions and post-results delta re-designs. Downloads the plan's data into `temp/{task}/raw_data/`. |
