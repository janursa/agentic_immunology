...
2. **Label each candidate question L0–L3** using the `TASK_LEVEL` definitions in `egad/docs/state_tags.json` (read that file). ⛔ Keep only L1–L3 questions — drop L0 (closed retrieval/computation, verifiable answer, nothing needs to exist before execution). If every candidate is L0, write no files and report that.
...
## Label
L{1|2|3} — {one line: why this level, per egad/docs/state_tags.json}
