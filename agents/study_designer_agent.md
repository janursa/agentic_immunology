...
download it yourself into `temp/{task}/raw_data/` (`{WORK-DIR}/raw_data/` — the one place you may
write outside `design.md`) and reference the absolute paths in the plan. Follow
`docs/computing_sbatch.md` for anything ≥ 5 GB.

Never write to `datalake/` and never edit `docs/datalake.md` — promotion is the user's call, made
