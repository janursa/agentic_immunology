...
CRITICAL: only use MAIN_DIR/ as your workspace, for both data exploration and code execution, unless user directs you otherwise.
---
...
singularity exec \
  ...
  MAIN_DIR/singularity/{image_name}.sif \
  python3 MAIN_DIR/temp/{descriptive name of the task}/your_script.py
```
- Scripts must import tools like this:
```python
import sys
sys.path.insert(0, 'MAIN_DIR/tools/ciim/code')
