...
Gated on content, not filename: fires on any write under ${CIIM_TEMP_DIR} whose
resulting text carries a `MODE: DESIGN-REVIEW` line and a CHECKS block. Fails open
otherwise (no canonical list, no mode line, no CHECKS block — so RESULTS-REVIEW
files pass straight through), which also keeps the reviewer able to fix a denied
write inside its own call rather than needing a re-dispatch.
