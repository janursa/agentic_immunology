...
project has its own feedback store (memory_bank/memory_blob.py) and the two
systems have been confused before — the orchestrator once wrote a feedback
lesson into the auto-memory folder instead of calling memory_blob.py. Deny
and redirect.
...
REASON = (
    ...
    "are blocked in this project. Feedback/lessons belong in memory_bank/memory_blob.py "
    "(`python memory_bank/memory_blob.py add --issue-tag <tag> --agents <agent1,agent2> "
    "--task <task> --lesson \"Situation: ... Lesson: ...\"`)."
)
...
def _demo() -> None:
    ...
    assert block_reason("Write", "memory_bank/memory_blob.jsonl") is None  # project's own store
