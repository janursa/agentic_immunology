...
- **Past analyses**: `past_analysis/index.jsonl` — one line per archived task (prompt, datasets, findings, paths).
...
| what | where |
|---|---|
| provisioned datasets | `datalake/` — indexed in `docs/datalake.md` |
| downloaded for this task | `temp/{task}/raw_data/` |
| derived, reused across phases | `temp/{task}/processed_data/` |
| per-phase outputs | `temp/{task}/phase_{n}/{sub_task}/results/` |
...
⛔ HARD RULE — `datalake/` and `docs/datalake.md` are never written by an agent mid-task. Promoting
...
2. **Document the analysis (ask, don't assume)** — ask whether to archive the run. Only on a yes:
   ...
   ```
   python scripts/archive_analysis.py {task} --prompt "{user's original prompt, verbatim}" \
       --level L{n} --datasets {comma-separated} --finding "..." --finding "..."
   ```
   Moves `temp/{task}/` to `past_analysis/{task}/` and appends one line to
   `past_analysis/index.jsonl`. 3–5 findings, each one sentence. Report the new paths — every link
   you gave the user earlier now points at the archived location.

   Also ask, separately, whether any dataset downloaded into `temp/{task}/raw_data/` should be
...
- ⛔ HARD RULE — `WORK-DIR` is the **exact** folder the agent writes into. Subagents never invent subfolders under it; you pick the path:
  ...
  - `peer_reviewer_agent` → `temp/{task}/phase_{phase}/design_review/` or `.../results_review/`
  - `data_analyst_agent` → `temp/{task}/phase_{phase}/{analysis}_{sub_task}/`
