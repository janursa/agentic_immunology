...
and the always-on hook (`.claude/hooks/check_guardrail_flag.py`). These are
...
1. Seed a canary lesson:
   `python knowhow/memory_blob.py add --issue-tag <tag> --agents echo_stub_agent --task probe --lesson "Situation: probe. Lesson: canary-lesson-xyz."`
