...
## Findings compared
PAPER (from CASE-CARD, verbatim):
- {bullet}
RUN (from report.md ## Summary, verbatim):
- {claim}

## Recovered
- {paper finding} → {run claim that carries it}
...
## Score
RECOVERED: {n}/{total paper findings}  (REPORTED-WEAKER counts here, flagged)
NOVEL: {n}
ROOT-CAUSE TALLY: {tag: count, ...}

## Summary
{most important convergence or divergence first}
```
