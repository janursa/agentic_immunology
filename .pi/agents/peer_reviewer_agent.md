...
- **Content relevance and robustness** — only if `GUARDRAIL: on`: check if the proposed plan addresses the relevant guidelines in `knowhow/guardrail.md`. If `GUARDRAIL: off`, skip this check entirely.
...
GUARDRAIL CHECK (knowhow/guardrail.md, if GUARDRAIL: on):
- {guideline}: relevant? Y/N -> addressed? Y/N/PARTIAL/N-A -> {one-line evidence or reason}
