...
Per-TF aging statistics across 5 major immune cell types (CD4T, CD8T, NK, B, MONO) from multi-cohort TF activity analysis (aging cohorts: onek1k, abf300, aida, perez_sle; healthy donors only). 
...
### `sle/sle_tf_signatures_major_celltypes.csv`
*SLE TF Signatures — Major*

Differential TF activity statistics for SLE vs healthy across 5 major immune cell types (CD4T, CD8T, NK, B, MONO) from the Perez cohort.

Columns: `gene`, `p_value`, `slope`, `ctrl`, `condition`, `p_value_adj`, `age_group`, `dataset`, `comparison`, `cell_type`.

**Load:** `retrieve_summary_stats('sle')`

---

### `sle/sle_gene_expression_signatures_major_celltypes.csv`
*SLE Gene Expression Signatures — Major*

Differential gene expression statistics for SLE vs healthy across 5 major immune cell types (CD4T, CD8T, NK, B, MONO) from the Perez cohort.

Columns: `gene`, `p_value`, `slope`, `ctrl`, `condition`, `p_value_adj`, `age_group`, `dataset`, `comparison`, `cell_type`.

**Load:** `retrieve_summary_stats('sle', feature_type='gene_expression')`

---

### `sle/sle_tf_signatures_sub_celltypes.csv`
*SLE TF Signatures — Sub Cell Types*

Differential TF activity statistics for SLE vs healthy across 14 sub immune cell types (Tcm_Naive_CD4, Tem_Effector_CD4, Treg, Tcm_Naive_CD8, Tem_Trm_CD8, Tem_Temra_CD8, MAIT, CD16_NK, Classic_MONO, NonClassic_MONO, Naive_B, Memory_B, Plasma_B, Aged_B) from the Perez cohort.

Columns: `gene`, `p_value`, `slope`, `ctrl`, `condition`, `p_value_adj`, `age_group`, `dataset`, `comparison`, `cell_type`.

**Load:** `retrieve_summary_stats('sle', cell_resolution='minor')`

---

### `sle/sle_gene_expression_signatures_sub_celltypes.csv`
*SLE Gene Expression Signatures — Sub Cell Types*

Differential gene expression statistics for SLE vs healthy across 14 sub immune cell types (same set as TF sub above) from the Perez cohort.

Columns: `gene`, `p_value`, `slope`, `ctrl`, `condition`, `p_value_adj`, `age_group`, `dataset`, `comparison`, `cell_type`.

**Load:** `retrieve_summary_stats('sle', feature_type='gene_expression', cell_resolution='minor')`
