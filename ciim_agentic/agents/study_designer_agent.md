...
## Data provisioning
The plan's data must exist before execution. For anything not already in `docs/datalake.md`,
download it yourself into `${CIIM_TEMP_DIR}/{task}/raw_data/` (`{WORK-DIR}/raw_data/` — the one place you may
write outside `design.md`) and reference the absolute paths in the plan. Follow
`docs/computing_sbatch.md` for anything ≥ 5 GB.

Never write to `${CIIM_DATALAKE_DIR}/` and never edit `docs/datalake.md` — promotion is the user's call, made
after the analysis (`docs/data_integration_format.md`). If a required dataset cannot be resolved or
downloaded, say so in the plan; do not design around data you have not confirmed exists.
...
(`${CIIM_MAIN_DIR}/scripts/render_review_artifact.py`) turns each `## ` section into its own reviewable card and drops
