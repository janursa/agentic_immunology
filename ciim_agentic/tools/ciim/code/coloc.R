...
.DATALAKE_DIR <- Sys.getenv("CIIM_DATALAKE_DIR")
if (.DATALAKE_DIR == "") stop("CIIM_DATALAKE_DIR is not set")
...
parse_args <- function() {
  ...
  p <- list(
    ...
    outdir      = NULL,
    ## GWAS
    gwas_file   = file.path(.DATALAKE_DIR, "gwas/GCST003156_SLE_Bentham2015.h.tsv.gz"),
    gwas_n      = 14267L,
    gwas_s      = 5201 / 14267,   # proportion cases (Bentham 2015)
    gwas_type   = "cc",
    ## eQTL
    eqtl_dir    = file.path(.DATALAKE_DIR, "dice/eqtls/full_summary_stats"),
    eqtl_n      = 91L,
    eqtl_sdy    = 1,
    ...
    col_pval    = "pval",
    ## LD reference
    kg_bfile    = file.path(.DATALAKE_DIR, "gwas/1kg/EUR"),
    plink       = "plink"
  )
