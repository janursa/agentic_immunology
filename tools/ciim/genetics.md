...
Python wrapper around `temp/coloc/coloc.R`. Runs GWAS × eQTL colocalization for a single locus using **coloc.abf** (all cell types) and **coloc.susie** (top N cell types). Executed inside `singularity/genotype.sif` (R 4.5, coloc, susieR, plink).
...
singularity exec \
  ...
  agentic_immunology/singularity/genotype.sif \
  Rscript agentic_immunology/tools/ciim/code/coloc.R \
    --gene IRF5 --chr 7 --pos 128954129 \
    --lead_rsid rs10488631 \
    --outdir /tmp/coloc_irf5
```
...
> **Replaces** the deprecated `query_opentarget` in `tools/biomni/database_biomni.py`.  
...
Python wrapper around `tools/ciim/code/mr.R`. Runs Mendelian Randomization in two modes. Executed inside `singularity/genotype.sif` (R 4.5, data.table, ggplot2, plink). MR methods implemented from scratch: Wald ratio (n=1), IVW (n≥2), MR-Egger (n≥3), Weighted Median (n≥3).
...
**OpenGWAS note:** JWT token must be stored in `agentic_immunology/.env` as `OPENGWAS_TOKEN=<jwt>`. Tokens expire ~2 weeks after issue; renew at https://api.opengwas.io/
