...
_CELLTYPIST_SUB_MAPPING = {
    ...
    'NK cells': 'NK',
    'Proliferating NK cells': 'CD16_NK',
...
def annotate_celltype_celltypist(
    ...
    major_ct_col='Major_CT',
    sub_ct_col='Sub_CT',
):
    """Annotate immune cell types in scRNA-seq data using CellTypist with majority voting.

    Loads an h5ad file, detects whether X contains raw counts or a 'counts' layer
    is available, normalizes to 10k CPM + log1p on a copy, and runs CellTypist
    (majority_voting=True) using the specified model.  Raw counts are preserved in
    the output.  Major and sub cell type labels are mapped from CellTypist predictions
    using standard immune cell nomenclature and stored in obs columns.

    Parameters
    ----------
    adata_path : str
        Path to input h5ad file (must contain raw integer counts in X or a 'counts' layer).
    output_dir : str
        Directory where annotated h5ad is saved.
    model : str
        CellTypist model name (default: 'Immune_All_Low.pkl').
    major_ct_col : str
        obs column name for major cell type labels (default: 'Major_CT').
    sub_ct_col : str
        obs column name for sub cell type labels (default: 'Sub_CT').

    Returns
    -------
    str
        Summary of steps performed and path to saved output file.
    """
    ...
    steps = []
    steps.append(f'Loading adata from {adata_path}')
    adata = sc.read_h5ad(adata_path)
    steps.append(f'Loaded: {adata.n_obs} cells x {adata.n_vars} genes')

    # --- Detect raw counts ---
    def _is_count_matrix(X):
        mat = X[:min(500, X.shape[0])]
        arr = mat.toarray() if sp.issparse(mat) else np.array(mat)
        return bool(np.all(arr >= 0) and np.allclose(arr, np.round(arr)))

    if 'counts' in adata.layers:
        steps.append("Using raw counts from adata.layers['counts']")
        raw_X = adata.layers['counts']
    elif _is_count_matrix(adata.X):
        steps.append('adata.X contains raw integer counts — using directly')
        raw_X = adata.X
    else:
        raise ValueError(
            "Could not find raw counts: adata.X does not appear to be integer counts "
            "and no 'counts' layer is present."
        )

    # --- Normalize on a copy (preserve raw counts in output) ---
    steps.append('Normalizing: total-count normalize to 10k CPM then log1p')
    adata_norm = sc.AnnData(X=raw_X.copy(), obs=adata.obs.copy(), var=adata.var.copy())
    sc.pp.normalize_total(adata_norm, target_sum=1e4)
    sc.pp.log1p(adata_norm)

    # --- CellTypist annotation ---
    steps.append(f'Running CellTypist (model={model}, majority_voting=True)')
    models.download_models(force_update=False)
    model_obj = models.Model.load(model=model)
    predictions = celltypist.annotate(adata_norm, model=model_obj, majority_voting=True)
    predicted = predictions.to_adata().obs['majority_voting']

    # --- Map to Major / Sub CT ---
    adata.obs[major_ct_col] = predicted.map(
        lambda x: _CELLTYPIST_MAJOR_MAPPING.get(x, 'Others')
    ).values
    adata.obs[sub_ct_col] = predicted.map(
        lambda x: _CELLTYPIST_SUB_MAPPING.get(x, 'Others')
    ).values
    adata.obs['celltypist_label'] = predicted.values

    dist = adata.obs[major_ct_col].value_counts().to_dict()
    steps.append(f'Cell type distribution ({major_ct_col}): {dist}')

    # --- Save ---
    os.makedirs(output_dir, exist_ok=True)
    out_path = os.path.join(output_dir, 'annotated_celltypist.h5ad')
    adata.write_h5ad(out_path, compression='gzip')
    steps.append(f'Saved annotated adata to {out_path}')
...
def infer_grn_spearman(
    ...
    *tf_list_path*.
    ...
    # ── prior paths ──────────────────────────────────────────────────────────
    PRIOR_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'data_lake', 'ciim')
    if tf_list_path is None:
        tf_list_path = os.path.join(PRIOR_DIR, "tf_all.csv")
    skeleton_path = os.path.join(PRIOR_DIR, "skeleton_promotor.csv")
