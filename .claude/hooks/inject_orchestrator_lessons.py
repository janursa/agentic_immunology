...
sys.path.insert(0, str(PROJECT_DIR / "memory"))
...
def build_context() -> str | None:
    ...
    if not entries:
        return None
    return (
        "Past lessons for you (orchestrator, from memory/memory_blob.jsonl):\n"
...
def _demo() -> None:
    context = build_context()
    assert context is not None, "no orchestrator lessons found in memory/memory_blob.jsonl"
