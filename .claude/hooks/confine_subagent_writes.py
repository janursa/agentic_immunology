...
and data_analyst_agent may only write/edit files under temp/ — mirrors
restrict_knowhow_access.py's read-side confinement, same RESTRICTED_AGENTS set, same
reasoning (ciim_agentic.md 'only use agentic_immunology/ as your workspace' plus the
data lake / knowhow being read-only to these agents).
...
def block_reason(agent_type: str, tool_name: str, tool_input: dict) -> str | None:
    ...
    try:
        ...
        return (
            f"{agent_type} may only write/edit files under temp/ (blocked path: {resolved}) — "
