...
`.sif` images live in `agentic_immunology/singularity/` (a symlink to `/vol/projects/CIIM/singularity`);
their `.def` build recipes are git-tracked in `singularity_docs/` (see `singularity_docs/build.sh`).

| Image | Path | Use for |
|-------|------|---------|
| `biomni_full.sif` | `agentic_immunology/singularity/biomni_full.sif` | Default — full bio stack (genomics, pharmacology, ML, NLP); `genetics_biomni`, `pharmacology_biomni`, and direct-API CIIM genetics functions (`phewas_opengwas`, `query_gwas_catalog`, `query_opentarget_platform`, `get_disease_credible_sets`) |
| `genotype.sif` | `agentic_immunology/singularity/genotype.sif` | `run_coloc` and `run_mr` (R 4.5, `coloc`, `susieR`, `plink`) |
