...
- Path: `temp/{task}/findings.md`. One file for the whole task.
...
Use exactly these `##` headers, in this order — `scripts/render_review_artifact.py` turns every `## `
...
- Images in "Detailed findings" must use markdown `![alt](path)` with the path **relative to
  `findings.md`'s own directory** — the HTML render is served from `temp/`, so an absolute filesystem
